...
**Ditemukan sekaligus dicatat, bukan diselesaikan sesi ini** (tersisa dari laporan rekonsiliasi awal sesi): dua dokumen sumber tidak konsisten status persetujuannya sendiri (`HS-TASKS-RIGHTCLICK-v1.1` = "Final, Gate G3 disetujui COO"; `HS-DB-RIGHTCLICK-v1.0` = "Draft — menunggu approval COO", gate yang SAMA) — perlu klarifikasi COO, bukan asumsi. Desain piutang/hutang aktual (§11 T5.3/T5.5) menyimpang dari `HS-DB-RIGHTCLICK-v1.0` §4.6: tidak ada tabel `receivables`/`payables`/`payment_allocations`, sehingga kapabilitas "satu pembayaran dialokasikan ke banyak tagihan sekaligus" (FR-M11a-05) tidak ada — sudah berjalan konsisten dan teruji, tapi tetap deviasi dari spesifikasi Final yang belum disetujui ulang. Ops panel sinkronisasi (T5.12 asli), antrean review duplikat partner (T5.11 asli), dan wujud infrastruktur 3-node (WireGuard + logical replication, T5.10 asli) masih belum ada sama sekali. Penomoran T2.x–T5.x di seluruh dokumen ini masih self-derived, belum direkonsiliasi terhadap `HS-TASKS-RIGHTCLICK-v1.1` yang sebenarnya.
